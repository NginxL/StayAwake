...
测试通过 `AwakeChecks` 可执行程序运行，使用可注入的时钟和模拟进程驱动。请使用此脚本，而非 `swift test`；项目未定义 XCTest 测试目标。核心检查覆盖会话到期、时长调整、屏幕模式切换、启动失败、进程异常退出、时间格式、版本比较及跨显示器的菜单栏锚点选择。
...
在已解锁的桌面会话中，通过 Finder 打开 `.build/PanelChecks.app`，点击 **Run display checks（运行显示器检查）**。测试会在每块已连接屏幕上短暂打开并调整面板，检查 WindowServer 登记、屏幕边界、当前桌面空间、重复开关及显示器变化时的关闭行为，并将带时间戳的结果保存到 `.build/PanelChecks-results.json`。完成后关闭测试应用。这些检查需要真实图形界面会话，与无桌面的 CI 分开执行，不会切换已安装应用的防休眠状态。

发布前还应实际验证面板、菜单栏操作、Dock 入口、登录启动设置和更新流程。核心测试程序不自动覆盖这些界面与安装路径。

多显示器验证应分别点击每块屏幕的菜单栏图标，并在面板打开时切换屏幕。确认面板跟随被点击的图标，在同一屏幕再次点击时关闭，重排或断开显示器后仍可使用。验证范围应包含不同缩放比例和上下排列的显示器。
...
| `Sources/AwakeUI/` | 按显示器创建面板、尺寸调整、焦点和关闭行为 |
| `Sources/StayAwake/` | SwiftUI 面板、AppKit 菜单栏与 Dock 行为、偏好保存、登录启动和更新 |
| `Tests/AwakeCoreTests/` | 独立核心检查与可选防休眠请求集成检查 |
| `Tests/AwakeUITests/` | 验证真实显示器和面板生命周期的交互式测试应用 |
...
`MenuBarAnchor` 使用应用激活前捕获的鼠标位置选择显示器。`MenuBarPanelController` 在该显示器上创建真实面板，并将其限制在可见屏幕范围内。所有菜单栏副本和 Dock 使用同一条显示路径。重建面板时保留 SwiftUI 控制器，尺寸变化在布局结束后应用。点击外部、按 Esc、切换应用、显示器配置或桌面空间变化均会关闭面板。关闭或重建面板不会停止防休眠。所有坐标计算均使用屏幕逻辑点，并支持负数屏幕起点。
