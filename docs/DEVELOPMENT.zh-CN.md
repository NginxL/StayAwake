...
`MenuBarAnchor` 使用应用激活前捕获的鼠标位置选择显示器。如果状态栏按钮的窗口位于该显示器，面板直接以按钮定位；否则，使用临时透明窗口把镜像菜单栏的点击锚定到对应屏幕。关闭面板时会释放该窗口。显示器配置变化会关闭面板，下一次点击重新计算位置。所有坐标计算均使用屏幕逻辑点，并支持负数屏幕起点。
